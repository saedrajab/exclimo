...
4. `lib/email.ts` — nodemailer with a cached transport. `sendBookingEmail` goes to the dispatch inbox; `sendBookingConfirmation` sends the visitor their own copy. With `SMTP_*` env vars set it sends for real; without them it uses an Ethereal test inbox in development (emails captured, never delivered, preview URL logged) and throws in production — the client then shows an inline error with the phone number and email address (there is deliberately no `mailto:` fallback; it was removed as unprofessional).
5. `lib/confirmation-email.ts` — subject, plain-text body and branded HTML for the visitor's confirmation copy. Table layout with inline styles (the only markup that renders predictably across mail clients), site palette, and the display font falling back to the condensed stack from `globals.css` since Bebas Neue is unavailable in email. All interpolated values pass through `escapeHtml`. The confirmation is best-effort: `sendRideRequest` logs a failure and still returns `ok: true`, because dispatch already holds the request and reporting failure would be worse than a missing courtesy email.
